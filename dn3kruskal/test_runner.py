...
def run_tests_df(script_name, python_path = 'python3'):
    # Najdemo vse testne primere
    tests = sorted([i for i in os.listdir(tests_path) if i.endswith('in')])    
    n_tests = len(tests)
    records = []
    # Poženemo test za vsak testni primer
    for i in range(n_tests):
        record =run_test_df(script_name, python_path=python_path, test_nb=tests[i])
        records.append(record)
    
    
    return  records

def run_test_df(script_name, python_path = 'python3', test_nb = 0):
       ...
       # Odpremo .out testno datoteko in preberemo izhod
        ...
        with open(f'{tests_path}/{test_name}', 'r') as test_file:
            ...
            if r.returncode != 0:
                ...
                err_msg = errors
                print(f'Test {test_name} je sprožil napako: \n {err_msg}') 
                return {"Status" : "Error", "Message": err_msg,"Time":round(elapsed, 3), "Script": script_name}               
            ...
            # Če napake ni, potem izvlečemo rezultat in ga primerjamo z pravilnim
                ...
                # Preverimo, da je rezultat pravilen
                if result != output:
                    return {"Status" : "Incorrect Anwser","Time":round(elapsed, 3), "Message": f'Test {test_name} je vrnil napačen odgovor!', "Script": script_name}    
                                                     
                
                else:
                    return {"Status" : "Success", "Message": f'Test {test_name} uspešen', "Time":round(elapsed, 3), "Script": script_name} 
                    #print(f'Test {test_name} uspešen. Čas: {str(round(elapsed, 3))} sekunde.')

    else:
        return {"Status" : "File dont exists","Time":round(elapsed, 3), "Message": f'Testna datoteka {test_name} ne obstaja!', "Script": script_name}    
        #print(f'Testna datoteka {test_name} ne obstaja!')
